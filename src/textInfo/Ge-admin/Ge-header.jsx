const header = {
    lang: "GEO",
    modalUserType:{
        selectAccountTypeText: 'აირჩიეთ თქვენი აკაუნტის ტიპი',       
    },
    modalCountrySelect:{
        selectCountryText: 'შეარჩიეთ ქვეყანა, რომ მოძებნოთ მძღოლები და სახელმძღვანელო'
    },
    menuElements:[
        "მარშრუტის კონსტრუქტორი","მარშრუტები","ადგილები","ტურები", 'სახელმძღვანელო',"მძღოლები"
    ],
    burgerMenu:{
        more:"მეტი",
        profile: 'პროფილი',
        trips: 'ჩემი მოგზაურობები',
        settings: 'პარამეტრები',
        partnership: 'პარტნიორული პროგრამა',
        exit: 'გამოსვლა',
        enter: 'შესვლა',
        burgerEnter:"შესვლა ან პროფილის შექმნა",
        titlesName:["პარამეტრები","მომსახურება","ანგარიში","სასარგებლო ბმულები"],
        settingsDrop:["ენის შეცვლა","ვალუტის შეცვლა"],
        services:["მარშრუტები","ტურები","ადგილები",'სახელმძღვანელო',"მძღოლები"],
        usefulLinks:["მომსახურების შესახებ"," პარტნიორებისთვის","სალიცენზიო ხელშეკრულება","დახმარება","საკონტაქტო ინფორმაცია"],
    },
    toPageStart: 'გვერდის დასაწყისში',
    loading: 'იტვირთება',
    noCommentsText: 'ამ დროისთვის კომენტარი არ არის',
    monthArray:['იანვარი', 'თებერვალი', 'მარტი', 'აპრილი', 'მაისი', 'ივნისი',
    "ივლისი", "აგვისტო", "სექტემბერი", "ოქტომბერი", "ნოემბერი", "დეკემბერი"]
}

export default header;